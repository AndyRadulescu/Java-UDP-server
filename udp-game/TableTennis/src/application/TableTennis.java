...
public class TableTennis extends Application implements Sizes, Values, Runnable {
    ...
    private Parent createContent() {
        ...
        KeyFrame frame = new KeyFrame(Duration.seconds(time), event -> {
            ...
            try {
                ...
                Message message = new Message(SETALL, rect1Y, rect2Y, ballX, ballY, height, width, ballSpeed);
                ClientServer.sendAllPacket(message, clientSocket);
    ...
    private void setAll(String message) {
        ...
        if (parts[0].compareTo("1") == 0 || parts[0].compareTo("7") == 0) {
            ...
            //  System.out.println(rect2.getTranslateY());
        }
    }
    ...
    private void bounce(Rectangle rect1, Rectangle rect2) {
        ...
        if ((ball.getTranslateX() >= canvasSizeX - 25) && (ball.getTranslateY() >= rect2.getTranslateY() - 10
            ...
            calculate(value2);
            if (ballSpeed + increaseSpeed < maxBallSpeed) {
                ballSpeed += increaseSpeed;
            }
            score += 100;
            label.setText("Score:" + String.valueOf(score));
        }
        ...
        scene.setOnMouseDragged(e -> {
            if (e.isPrimaryButtonDown()) {
                if (e.getSceneY() < mousePointer) {
                    ...
                    if (rect2.getTranslateY() <= canvasSizeY - rectYsize) {
                        if (clients[0]) {
                            rect2Y = e.getSceneY();
                        }
                    }
    ...
    public void start(Stage primaryStage) {
        try {
            ...
            if (receivedMessage.compareTo("0;Received! safoddsfsadf sdaf sadf asd;") == 0) {
                Message message1 = new Message(SETCLIENT);
                ClientServer.setClient(message1, clientSocket);
                String setClient = ClientServer.receivePacket(clientSocket);
                setClient(setClient);
                Message message2 = new Message(FIRSTSET);
                ClientServer.sendAllPacket(message2, clientSocket);
